...
#
# same as experiment4 but number of restarts is set to 15.
#
# Run on hephaistos
#
...
outparallel = @showprogress pmap(tfarray->(@suppress performcv(tarray=tobs, yarray=yobs, stdarray=σobs, kernelname="matern32", tfarray=tfarray, iterations=2500, numberofrestarts=15, ρmax=10.0)), T)

# save results

prob = getprobabilities(outparallel)

JLD2.save("experiment5.jld2", "out", outparallel, "prob", prob, "lambda", lambda, "masses", masses, "eddingtonfraction", 10.0, "truemass", 2e8)
